...
public class MoneyStipendPlugin extends AbstractOperationServicePlugIn {
        ...
        try {
            for (DynamicObject single : e.getDataEntities()) {
                ...
                AddMoneyManageFunction.addMoneyManage(
                    ...
                    String.format("任务编号:%s,任务名称:助学金发放",single.getString("billNo")));
            }
        } catch(Exception ee) {
            for (ExtendedDataEntity entity : e.getSelectedRows()) {
                String billNo = entity.getBillNo();
                //获取发放单信息
                DynamicObject single = BusinessDataServiceHelper.loadSingle("ozwe_stipendput",
                        "billno," +
                        "ozwe_putcard," + 
                        "ozwe_amountfield",
